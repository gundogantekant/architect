...
- **Before creating a worktree, verify `projectPath` is the repository root** (`git rev-parse --show-toplevel` must match `projectPath`). This guards against creating a worktree in architect's git context when the caller's cwd was wrong — the most common cause of misplaced worktrees.
...
- **Contract-satisfied auto-merge**: When `POST /api/dispatch/:id/complete` is called and `contract_satisfied = true` + `scope_violation = false` + `merge_gate = "auto"`, the server sets status to `merge_pending` and calls `triggerMerge` immediately — no user confirmation. This routes through `merge_pending` so `restoreSessions` can resume interrupted merges on restart. After a successful merge: `merged_at` and `merge_target` (the originating branch) are recorded on the dispatch; the worktree is removed (`git worktree remove --force`, with `rm -rf` fallback if the directory still exists); the local branch is force-deleted (`git branch -D`). Confirmation is required when: a merge conflict is present, `scope_violation = true`, `contract_satisfied` is false/null, or `merge_gate ≠ "auto"`.
