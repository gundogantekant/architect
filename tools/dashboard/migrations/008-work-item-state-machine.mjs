/**
 * Migration 008: Work item state machine redesign.
 *
...
export function up(db) {
  ...
  db.exec(`
    ALTER TABLE work_items ADD COLUMN input_needed INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE work_items ADD COLUMN input_needed_from TEXT;
    ALTER TABLE work_items ADD COLUMN input_needed_reason TEXT;
    ALTER TABLE work_items ADD COLUMN input_needed_at TEXT;

    ALTER TABLE work_items ADD COLUMN approval_active INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE work_items ADD COLUMN approval_mode TEXT DEFAULT 'all'
      CHECK(approval_mode IN ('all','any','sequential'));
    ALTER TABLE work_items ADD COLUMN approval_requested_at TEXT;
    ALTER TABLE work_items ADD COLUMN approval_resolved_at TEXT;

    ALTER TABLE work_items ADD COLUMN released_at TEXT;
    ALTER TABLE work_items ADD COLUMN released_version TEXT;
  `);
  ...
  try {
    db.exec(`
      ...
      CREATE INDEX idx_wia_work_item ON work_item_approvals(work_item_id);
      CREATE INDEX idx_wia_identity_status ON work_item_approvals(identity, status);
      CREATE INDEX idx_wia_blocking ON work_item_approvals(blocking_work_item_id)
