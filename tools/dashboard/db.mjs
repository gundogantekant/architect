...
/**
 * Build the GROUP BY label expression and JOIN clause for a given groupBy key.
 * Returns { labelExpr, joinClause } strings for use in the summary query.
 */
...
/**
 * Group cost rows by calendar day for trend output.
 * Returns [{ period, cost_usd, sessions }] sorted ascending.
 */
 ...
 *
 * Returns total cost, tokens, sessions, a breakdown by the requested dimension,
 * and a day-by-day trend for the selected date range.
 *
 * @param {object} params
 * @param {string} [params.from]     ISO date string (inclusive). Defaults to 30 days ago.
 * @param {string} [params.to]       ISO date string (inclusive). Defaults to today.
 * @param {string} [params.groupBy]  One of: model | agent_role | project_key | epic_id
 */
export async function getCostSummary({ from, to, groupBy = 'model' } = {}) {
  ...
  // Totals query — always uses full date range
  ...
  // Breakdown query — grouped by the requested dimension
  ...
  // Raw rows for trend computation
