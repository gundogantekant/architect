...
describe('validateContract', () => {
  ...
  it('returns violations for medium with missing e2e_test_criteria', () => {
    ...
    const violations = validateContract(item, { ...baseContract });
    assert.ok(violations?.some(v => v.field === 'e2e_test_criteria'));
  });

  it('passes medium with 1 e2e criterion', () => {
    const item = { tags: ['medium'] };
    const result = validateContract(item, { ...baseContract, e2e_test_criteria: ['test A'] });
    assert.equal(result, null);
  });

  it('fails large with 2 e2e criteria', () => {
    const item = { tags: ['large'] };
    const violations = validateContract(item, { ...baseContract, e2e_test_criteria: ['a', 'b'], scope_boundary: 'tools/', stop_conditions: ['s1', 's2', 's3'] });
    assert.ok(violations?.some(v => v.field === 'e2e_test_criteria'));
  });

  it('passes large with 3 e2e criteria + scope_boundary + stop_conditions', () => {
    const item = { tags: ['large'] };
  ...
  it('fails large without scope_boundary', () => {
    const item = { tags: ['large'] };
    const violations = validateContract(item, { ...baseContract, e2e_test_criteria: ['a', 'b', 'c'], stop_conditions: ['s1', 's2', 's3'] });
    assert.ok(violations?.some(v => v.field === 'scope_boundary'));
  });

  it('fails large with fewer than 3 stop_conditions', () => {
    const item = { tags: ['large'] };
    const violations = validateContract(item, { ...baseContract, e2e_test_criteria: ['a', 'b', 'c'], scope_boundary: 'tools/', stop_conditions: ['s1', 's2'] });
