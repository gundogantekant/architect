// Child workflow stub for future isolated cloud domain EXECUTION tasks.
//
...
//
// The current MVP uses analyzeCloudImpact activity for cloud analysis.
// This child workflow is scaffolded for future use when cloud changes need
// a full Claude Code session (edit files, run infra commands, apply Terraform).
...
// Dispatches a full Claude Code session for cloud domain execution work.
//
...
// Call via: workflow.startChildWorkflow(cloudEvalWorkflow, input, { taskQueue: 'user-session' }).
export async function cloudEvalWorkflow(input: CloudEvalInput): Promise<CloudEvalResult> {
  const result = await dashboardDispatch({
    workItemId: input.workItemId,
    projectKey: input.projectKey,
    permissionMode: 'acceptEdits',
    additionalInstructions: `Cloud domain execution: ${input.requirement}${
