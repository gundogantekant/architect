...
5. **Plan implementation**: Produce a bullet-point plan (max 7 points) covering: files to modify/create, approach summary, test strategy, **acceptance criteria** (1–3 sentences stating what must be observably true when done), and **E2E test scenarios** (specific behaviors to validate end-to-end; required for medium+, best-effort for trivial/small). If the work item has a stored `plan.md` artifact, use it as the basis instead of generating from scratch. Do NOT present to user yet — board review runs first in step 6.
